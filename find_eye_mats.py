...
def find_eye_materials(project_path):
    print("Finding eye materials in scene...")
    ...
    # 2. List materials for each and look for 'eye'
        ...
        try:
            with open(os.path.join(outbox, f"res_list_{id}.json"), 'r') as f:
                ...
                for m in mats["materials"]:
                    if "eye" in m["name"].lower():
                        print(f"FOUND: Object '{name}' (ID: {id}) has material '{m['name']}' at index {m['index']}")
        except:
            pass

if __name__ == "__main__":
    find_eye_materials("/home/bamn/ALCOM/Projects/BAMN-EXTO")
